...
def merge_results(results_folder, master_file):
    ...
    # 2. Identify Result Columns vs. Parameter Columns
    # These are the columns we want to aggregate (sum) 
    val_cols = ['num_shots', 'num_switches']
    
    # Calculate absolute errors to allow for weighted averaging 
    df_all['total_errors'] = df_all['LER'] * df_all['num_shots']
    
    # Every column that is NOT a result is a 'Key'. 
    # This ensures that if even one parameter (like strong_num_sets) 
    # is different, the rows stay separate.
    keys = [c for c in df_all.columns if c not in val_cols + ['LER', 'total_errors']]

    # 3. Group and Aggregate
    # We sum shots and errors for identical parameter sets.
    # dropna=False ensures that rows with empty/NaN values aren't deleted.
    agg_logic = {col: 'sum' for col in val_cols + ['total_errors'] if col in df_all.columns}
    
    grouped = df_all.groupby(keys, dropna=False).agg(agg_logic).reset_index()

    # 4. Final Weighted LER Calculation
    grouped['LER'] = grouped['total_errors'] / grouped['num_shots']
    
    # 5. Column Cleanup and Save
    # Ensure LER is the first column for readability
    cols_to_keep = ['LER'] + [c for c in grouped.columns if c not in ['LER', 'total_errors']]
    updated_master = grouped[cols_to_keep]
    ...
    print(f"Merge Complete! Processed {len(cluster_files)} batch files.")
